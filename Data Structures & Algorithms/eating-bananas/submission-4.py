...
class Solution:
    """
    It turns out that my approach is really complicated and perhaps not even better in terms of complexity 
    than a simple binary search.

    What I agree on with the solution:
    - There is an upper bound for k (max(piles)).
    - And we somehow need to find the lowest k for which we hit the time goal

    What the solution did better than me:
    - [Minor] Not necessary to write an additional function for evaluate k. This can be a two-line for loop.
    - [More significant] Solution offers a better implementation of binary search, where we want to find a lower bound:
        - Normal handling of lower bound, upper bound, midpoint
        - Keeps track of the lowest value for which we get a match. 
          Then even if we get a match, we still simply half the search space (lower half),
          - again, keeping score of that match.

    Interesting ideas I had:
    - There is also a lower bound for k. That lower bound is ceil(sum of all the bananas / k)
    - Perhaps we can use the fact that our feedback signal is a little bit richer than in normal binary search
      (where we simply get higher/lower feedback) to multiply k by around (hours of current k) / (hours goal).
      I think this is potentially a very good idea - but I found this difficult to implement.
      (And the fact that the dependence of the hours on k is not obvious at all (because of the remainders)
      makes this really difficult to implement efficiently.)
      A downside of this idea that I recognised: there could be a series of many values of k which all lead to the
      same time which is just above our hours goal. In that case, our updates along that series of k might take
      very long.
    
    Lessons:
    - It's probably never optimal to implement two search algorithms here. Just choose one. This will probably
      be same or better complexity, unless I have a very good understanding of how the first algorithm affects
      the range I search when using the second algorithm.
    """
    ...
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        # It's quite difficult to analyse the complexity of my solution. But arguably, in the
        # worst case, it could probably be linear in the number of possible solutions 
        # (due to the first stage, where we try to get the range of the ks down to something sensible).
